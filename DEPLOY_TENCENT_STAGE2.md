...
1. 速记会话合并：`git pull` 后**必须同时**更新插件与核心——
   ...
   要求插件与核心同时更新到本轮版本。
2. Web「AI 整理」：节点详情页有单节点整理按钮；思维树页的按钮升级为
   **「AI 深度整理全树」**——逐层遍历整棵树，凡子节点 ≥2 且自上次整理后
   子树有过变更的层级都做一次分析（无变更的层级自动跳过；每次点击最多
   分析 8 个层级，超出的下次点击继续，提示语会说明）。分析结果只产生
   **待确认**的内容，全部集中在待确认箱：合并建议是普通候选卡，AI 关系
   显示在同页的「AI 关系待确认」区块（也可在节点详情页处理）。
